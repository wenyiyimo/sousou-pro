...
function getSeekNum(num)
  local hisSeekNum = 0
  if #historys > 0 and historys[1][1] == File(videos[1]).getParent() and historys[1][2] == play_name and videoDuration-historys[1][3] >3000 then

    hisSeekNum = historys[1][3]
    -- hisSkipFlag = false
    if historys[1][4] then
      playRate=historys[1][4]
    end
      ...
      if playbackState == Player.STATE_ENDED then
       ...
       elseif playbackState == Player.STATE_READY then
        ...
        if prepared then
          ...
          -- if setting.getBoolean('autoFullScreen', false) then
          setPlayFullscreen()
  ...
  if prepared then
    ...
    if videoPlayer.isPlaying() then
      ...
      if playCurrent % 10 == 0 and playCurrent>3000 then
        ...
        -- }
        task(100,function()
          historys = videoUtil.setData({temp.getParent(), play_name, playCurrent,playRate})
        end)
