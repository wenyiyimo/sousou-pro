...
--import"xyz.doikki.videoplayer.exo.MyHlsExtractorFactory"
...
--import "com.sousou.utils.FfmpegRenderersFactory"
import "androidx.media3.exoplayer.trackselection.DefaultTrackSelector";
...
loadControl = DefaultLoadControl.Builder().setAllocator(DefaultAllocator(true, C.DEFAULT_BUFFER_SEGMENT_SIZE))
.setTargetBufferBytes(C.LENGTH_UNSET)
.setBufferDurationsMs(60000, 100000, 100, 1000)
.setPrioritizeTimeOverSizeThresholds(true).build()
--import "io.github.anilbeesetti.nextlib.media3ext.ffdecoder.NextRenderersFactory"
--import "io.github.anilbeesetti.nextlib.media3ext.ffdecoder.FFmpegOnlyRenderersFactory"
--defaultRenderersFactory = NextRenderersFactory(this)




--defaultRenderersFactory = FFmpegOnlyRenderersFactory(this)


defaultRenderersFactory = DefaultRenderersFactory(this);
--defaultRenderersFactory = FfmpegRenderersFactory(activity.getContext());
...
videoPlayer = SimpleExoPlayer.Builder(this,defaultRenderersFactory)
...
mUserAgent="Mozilla/5.0 (Linux; Android 7.1.2; Build/NJH47F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.109 Safari/537.36"

dataSourceFactory = DefaultDataSourceFactory(this, Util.getUserAgent(this,mUserAgent));
--okBuild=OkHttpClient.Builder()
--builder.connectionSpecs(Util.immutableList(RESTRICTED_TLS, MODERN_TLS, COMPATIBLE_TLS, CLEARTEXT));
--okBuild.retryOnConnectionFailure(true);
--okBuild.followRedirects(true);
--okBuild.followSslRedirects(true);
--sslSocketFactory = SSLCompat();
-- okBuild.sslSocketFactory(sslSocketFactory, SSLCompat.TM);
--  okBuild.hostnameVerifier(HttpsUtils.UnSafeHostnameVerifier);

--dataSourceFactory = DefaultDataSourceFactory(this, OkHttpDataSource.Factory(JavaUtils().initExoOkHttpClient().build())
...
--[=[if pcall(function()
    ...
    task(200,function()
      cache = SimpleCache(
      File("/sdcard/Download/搜搜/cache"),
      LeastRecentlyUsedCacheEvictor(1024 * 1024 * 1024*2),
      null
      )
...
.setUserAgent("Mozilla/5.0 (Linux; Android 7.1.2; Build/NJH47F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.109 Safari/537.36")
)
)
...
.setEventListener(CacheDataSource.EventListener {
  onCachedBytesRead=function(cacheSizeBytes,cachedBytesRead)
  end,

  onCacheIgnored=function(reason)
...
function startPlay(url,seekNum)
  ...
  if (not url:find '127.0.0') and (not url:find '192.168') and (not url:find '0.0.0') and cacheFactory then
  ...
  if pcall(function()
      ...
      if C.TYPE_OTHER == type then
        ...
        if (url:find 'm3u8') then
         ...
         -- mediaSource= DefaultMediaSourceFactory(dataSourceFactory, DefaultExtractorsFactory()
...
--mediaSource= DefaultMediaSourceFactory(dataSourceFactory, DefaultExtractorsFactory()).createMediaSource(MediaItem.fromUri(uri))
--mediaSource = SsMediaSource.Factory(dataSourceFactory).createMediaSource(MediaItem.fromUri(uri))
         ...
         -- mediaSource = HlsMediaSource.Factory(dataSourceFactory).setAllowChunklessPreparation(true)
         --.setExtractorFactory(MyHlsExtractorFactory()).createMediaSource(MediaItem.fromUri(uri))
          --showToast(111111)
        end
      ...
      if code == 200 and body:find("EXTM3U") then
        ...
        if pcall(function()
            ...
            if types == true then
              ...
              startPlay("/sdcard/搜搜/index.m3u8",seekNum)

              --end)
             else
              initPlayer(result[2][1][1],seekNum)
            end
          end) then
...
holder.addCallback(SurfaceHolder_Callback {
  ...
  surfaceChanged = function(holder, format, width, height)
    -- videoPlayer.getVLCVout().setWindowSize(surfaceView.getWidth(), surfaceView.getHeight());
    --videoPlayer.getVLCVout().attachViews();
  end,
